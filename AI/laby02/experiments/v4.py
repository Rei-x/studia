...
try:
    ...
    print(
        "The 'questionary' library is not installed. Please install it to use the enhanced UI:"
    )
    print("  pip install questionary")
    print("Falling back to basic input prompts.")
    questionary = None  # Fallback flag
...
# --- Minimax / AlphaBeta Algorithms (unchanged) ---
...
# --- Game Logic (verbose output during play unchanged) ---
def play_game(
    ...
    while True:
        ...
        if not possible_moves:
            ...
            sys.stderr.flush()  # Ensure "thinking" message appears before potential delay
        ...
        if current_player_char == BLACK:
            game_round_num += 1
        sys.stderr.flush()  # Flush after each turn's block of messages
    ...
    if winner_player:
        sys.stdout.write(
            f"Rounds: {int(num_rounds_played)}, Winner: Player {winner_player}\n"
        )
    else:
        sys.stdout.write(
            f"Rounds: {int(num_rounds_played)}, Winner: None (Game ended without a successful move or stalemate)\n"
        )
    sys.stdout.flush()
...
def get_player_settings_interactive(player_num_str: str, player_name: str):
    ...
    # Algorithm Choice
    algo_choices = ["minimax", "alphabeta"]
...
def get_player_settings_basic(player_num_str: str, player_name: str):  # Fallback
    print(
        f"\n--- Settings for Player {player_num_str} ({player_name}) ---",
        file=sys.stderr,
    )
    ...
    while chosen_heuristic_name not in HEURISTICS_MAP:
        prompt_text = f"Choose heuristic for Player {player_num_str} ({', '.join(HEURISTICS_MAP.keys())}): "
        chosen_heuristic_name = input(prompt_text).strip()
...
def main():
    if questionary:
        print(
            "Welcome to Clobber AI! (Enhanced UI with 'questionary')", file=sys.stderr
        )
        get_settings_func = get_player_settings_interactive
    else:
        # questionary is None, fallback to basic input
        get_settings_func = get_player_settings_basic

    default_rows, default_cols = 5, 6
    game_board_str = None
    rows, cols = default_rows, default_cols

    use_custom_board = False
    if not sys.stdin.isatty():  # Input is piped
        if questionary:
            confirm_piped = questionary.confirm(
                "Input seems to be piped. Use it as the game board?", default=True
            ).ask()
            if confirm_piped is None:
                sys.exit("Setup cancelled.")
            use_custom_board = confirm_piped
        else:  # basic mode, assume piped input is intentional
            use_custom_board = True
        ...
        if use_custom_board:
            ...
            if raw_board_input:
                ...
                rows = len(lines)
                if rows > 0 and lines[0].split():
                    cols = len(lines[0].split())
                    print(f"Read board {rows}x{cols} from stdin.", file=sys.stderr)
                else:
                    print(
                        "Empty or invalid board input from stdin, using default 5x6.",
                        file=sys.stderr,
                    )
                    rows, cols = default_rows, default_cols
                    game_board_str = None
                ...
                use_custom_board = False  # Revert if piped input was empty

    if not use_custom_board:  # No piped input, or user chose not to use it
        print("Using default 5x6 Clobber board.", file=sys.stderr)
        game_board_str = None  # Ensure default is used

    sys.stderr.flush()

    p1_algo, p1_heuristic, p1_depth = get_settings_func("1", "Black")
    p2_algo, p2_heuristic, p2_depth = get_settings_func("2", "White")
