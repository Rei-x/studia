...
class Board:
    ...
    def display(self, file=sys.stdout):
        ...
        file.flush()  # Ensure it's printed immediately
...
def h_adaptive_strategy(board: Board, player_char_perspective: str):
    ...
    if is_endgame:
        # sys.stderr.write(f"Debug: Adaptive (Player {player_char_perspective}): ENDGAME strategy\n")
        return (
    ...
    if piece_diff_val < -1:
        # sys.stderr.write(f"Debug: Adaptive (Player {player_char_perspective}): BEHIND strategy\n")
        return h_combined_piece_mobility(board, player_char_perspective)

    elif piece_diff_val > 1:
        # sys.stderr.write(f"Debug: Adaptive (Player {player_char_perspective}): AHEAD strategy\n")
        return (
        ...
        # sys.stderr.write(f"Debug: Adaptive (Player {player_char_perspective}): BALANCED MIDGAME strategy\n")
        return h_combined_piece_mobility(board, player_char_perspective)
...
# --- Alpha-Beta Pruning Algorithm (unchanged) ---
...
def play_game(
    ...
    # Calculate total algorithm time for decisions only
    ...
    while True:
        ...
        game_board.display(sys.stderr)  # Display current board to stderr
        ...
        if not possible_moves:
            ...
            if not opponent_moves:
                ...
                break  # Game Over, winner is last_successful_mover
            else:
                print(f"Player {current_player_char} passes turn.", file=sys.stderr)
                # Current player passes, last_successful_mover doesn't change
        else:
            ...
            total_decision_making_time += (
                time_taken_move_decision  # Accumulate decision time
            )
            ...
            if best_move is None:
                # This implies possible_moves was empty, which should be caught above.
    ...
    # In Clobber, the player who makes the last valid move wins.
    # If last_successful_mover is None, it means no moves were made (e.g., board too small or already terminal)
    winner_player = last_successful_mover

    # A "round" is typically one move by Black and one by White. ply_count is total moves.
    ...
    if winner_player:
        ...
        # This case could happen if the initial board has no moves for Black.
        sys.stdout.write(
            f"Rounds: {int(num_rounds_played)}, Winner: None (Game ended without a successful move or stalemate)\n"
        )
    sys.stdout.flush()

    # Standard Error Output for statistics
...
def get_player_settings(player_num_str: str, player_name: str):
    ...
    sys.stderr.flush()  # Ensure prompt is visible before input
