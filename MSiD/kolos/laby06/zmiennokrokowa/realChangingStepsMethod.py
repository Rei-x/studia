...
# def h_(x_i: np.ndarray):
#     x1, x2 = x_i[0], x_i[1]
#     def f(h):
#         return 2.5*((-10*(x1**3)*h + 10*x1*x2*h - 2*x1*h + x1 - 2*h)**2 -5*(x1**2)*h + 5*x2*h -x2)**2 + (1 + 10*(x1**3)*h -10*x1*x2*h + 2*x1*h - x1 + 2*h)**2
#     result = minimize_scalar(f, bounds=(0, 1000))
#     return result.x


def h_(x_i: np.ndarray):
    def f(h):
        return func(x_i - h * grad(x_i))
...
def main():
    ...
    if not algorithm.start_algorithm():
        ...
        print(
            f"Solution found after {i} iterations:\nminimal x: {x_min}\nfunction value: {func(x_min)}"
        )
