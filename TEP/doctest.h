...
#include <unistd.h>
                                    ...
                                    const char *file, int line)
        {
            ...
            s << Color::Cyan << assertString(at) << "( " << expr << ", " << as << " ) " << Color::None
              << (threw ? (threw_as ? "threw as expected!" : "threw a DIFFERENT exception: ") : "did NOT throw at all!")
              << Color::Cyan << ex << "\n";
            s << contextState->contexts;
        }
        ...
        ResultBuilder::ResultBuilder(assertType::Enum at, const char *file, int line, const char *expr,
                                     const char *exception_type)
            : m_assert_type(at), m_file(file), m_line(line), m_expr(expr), m_exception_type(exception_type), m_threw(false), m_threw_as(false), m_failed(false)
        {
#if DOCTEST_MSVC
